...
目前由 H_W_0 修改了部分，基本属于自用

修改内容包括：
- 实现 **Discord** 频道间互转发消息不报错
- 新增双语翻译及隐藏翻译提示文本
...
- `use_recent_context`：开启后，将同一来源会话的历史消息作为普通聊天翻译上下文，仅用于语义消歧；模型仍只输出当前消息的译文。该上下文仅保存在内存中，插件重启后清空。
- `recent_context_count`：普通聊天翻译时包含的前向上下文消息数量，范围为 0-20，默认 5；不影响合并转发记录翻译的同级上下文数量。

每条 `forward_rules` 规则的 `translation.translate_forward_records` 可控制是否翻译 QQ 合并转发记录中的文本内容；默认关闭。翻译合并转发记录时，会默认向 AI 提供同一嵌套层级中当前记录前后各最多 5 条同级记录作为背景，不会跨父子嵌套层级取上下文。图片、文件和引用元数据保持原样，翻译失败时回退到原文。
