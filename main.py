...
class MsgTransfer(star.Star):
    ...
    def _component_kind(component) -> str:
        """返回消息组件的稳定类型名，兼容 AstrBot 枚举和测试替身。"""
        raw_type = None
    ...
    def _node_children(cls, component) -> list:
        """提取 Node/Nodes 子节点，遇到畸形数据时返回空列表。"""
        ...
        for unit in self._build_merged_forward_units(message_chain):
            ...
            for component in unit.get("components", []):
                ...
                if kind == "plain":
                    ...
                    qq = component.get("data", {}).get("qq") if isinstance(component, dict) else getattr(component, "qq", "")
                    values.append(f"@{qq}" if qq else "[艾特]")
            value = "".join(values).strip()
            if value:
                text_lines.append(f"{self._merged_forward_header(unit)}: {value}")
            elif unit.get("placeholder"):
                text_lines.append(f"{self._merged_forward_header(unit)}: {unit['placeholder']}")
        return "\n".join(text_lines)

    def _format_merged_component_content(self, components) -> str:
        """复用现有格式化器，并为不支持的组件补充可读占位信息。"""
        ...
        rendered = DiscordWebhookManager.format_message_content(renderable_components, skip_images=True)
        extra_lines = []
        ...
        for component in components:
            ...
            if kind in {"record", "audio"}:
                source = getattr(component, "url", None) or getattr(component, "file", None) or getattr(component, "path", None)
                extra_lines.append(f"[语音]({source})" if isinstance(source, str) and source.startswith("http") else "[语音]")
                continue
            if kind == "video":
                source = getattr(component, "url", None) or getattr(component, "file", None) or getattr(component, "path", None)
                extra_lines.append(f"[视频]({source})" if isinstance(source, str) and source.startswith("http") else "[视频]")
                continue

            # Unknown components must remain visible instead of aborting the full record.
    ...
    async def _translate_forward_record_components(
        ...
        async def flush_text_buffer() -> None:
            ...
            try:
                ...
                logger.warning(f"[MergedForward] 转发记录文本翻译失败，回退原文: {e}")
            translated_components.append(Plain(text=translated_text or original_text))

        for component in components:
            if self._component_kind(component) == "plain":
                value = component.get("data", {}).get("text") if isinstance(component, dict) else getattr(component, "text", "")
    ...
    async def _forward_merged_forward_with_webhook(
        ...
        """创建独立 Thread，并按递归展开后的顺序发送合并转发节点。"""
        ...
        try:
            ...
            logger.error(f"[MergedForward] 创建 Thread 异常 #{rule_id}: {_classify_error(e)}")
            return False
        ...
        try:
            ...
            logger.warning(f"读取QQ名称映射失败，合并转发中的艾特将使用QQ号: {e}")
            mapping = {}
        ...
        for unit_index, unit in enumerate(self._build_merged_forward_units(message_chain), start=1):
            try:
                content, embeds, files = await self._prepare_merged_forward_unit(event, unit, rule or {}, mapping)
                ...
                for chunk_index, chunk in enumerate(chunks):
                    ...
                    if not sent_id:
                        all_sent = False
                        logger.error(
                            f"[MergedForward] Thread 消息发送失败 #{rule_id} (节点={unit_index}, 分片={chunk_index + 1})"
                        )
                ...
                all_sent = False
                logger.error(
                    f"[MergedForward] 处理 Thread 节点失败 #{rule_id} (节点={unit_index}): {_classify_error(e)}",
                    exc_info=True,
                )
        ...
        if first_discord_msg_id and message_id:
            try:
                ...
                logger.error(f"保存合并转发消息映射失败(不影响发送): {e}")
    ...
    def _replace_ats(
        ...
        for seg in message_chain:
            ...
            elif drop_replies and seg.__class__.__name__ in ("Quote", "Reply"):
                continue
            else:
